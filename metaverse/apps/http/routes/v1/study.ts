...
// a single sitting is capped so an abandoned timer can't bank fake hours
...
studyRouter.post("/start", userMiddleware, async (req, res) => {
  ...
  // an open session that was never stopped (closed tab, crash) is discarded:
  // we can't know how long they actually studied
...
studyRouter.get("/me", userMiddleware, async (req, res) => {
  ...
  res.json({
    activeSession: session ? { sessionId: session.id, startedAt: session.startedAt } : null
  })
})
